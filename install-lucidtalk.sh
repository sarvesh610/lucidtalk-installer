...
EOF
    ...
    # Create the P2P download script with improved retry logic
...
async function downloadViaP2P() {
    ...
    try {
        ...
        // Initialize corestore with proper key conversion
        const store = new Corestore('./lucidtalk-temp');
        drive = new Hyperdrive(store, Buffer.from(DISTRIBUTION_KEY, 'hex'));
        ...
        // Wait for peer connections and file synchronization with retry logic
        ...
        if (error.message.includes('Cannot read properties of null') || 
            error.message.includes('Unable to access P2P files')) {
